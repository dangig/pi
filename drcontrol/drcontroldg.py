...
#
#   Modified the original drcontrol.py v. 0.12c to create a light flashing program ('program1')
#
# ----------------------------------------------------------------------------
...
def set_relay():
    ...
    try:
        with BitBangDevice(cmdarg.device) as bb:
            ...
            # Action towards Halloween program!
            if cmdarg.relay == "program1":
                ...
                # Transition to "00", from right to left
                bb.port = int("A8", 16)
                time.sleep(0.1)
                bb.port = int("A0", 16)
                time.sleep(0.1)
